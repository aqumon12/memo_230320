...
public class PostBO {
	...
	public List<Post> getPostListByUserId(int userId, Integer prevId, Integer nextId) {
		...
		if (prevId != null) {
			...
			// reverse (5 6 7 => 7 6 5)
			Collections.reverse(postList); // reverse 후 저장까지 해준다
			// return
			return postList;
		} else if (nextId != null) {
			// 다음
	...
	// 이전 방향의 끝인지 확인
		...
		public boolean isNextLastPage(int nextId, int userId) {
			return nextId == postMapper.selectPostIdByUserIdAndSort(userId, "ASC");  // 같으면 끝, 아니면 끝 아님
		}
	...
	public void updatePost(int userId, String userLoginId,
		...
		// 업데이트 대상인 기존 글을 가져와본다. (validation, 이미지 교체시 기존 이미지 제거를 위해서)
